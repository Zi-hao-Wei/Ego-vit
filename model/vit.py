...
from model.masking_generator import TubeMaskingGenerator
import numpy as np 
...
class PretrainVisionTransformerEncoder(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
                 ...
                 use_learnable_pos_emb=False,mask_ratio=0.5):
        super().__init__()
        self.img_size = img_size
        self.patch_size = patch_size
    ...
    def forward_features(self, x, mask_ratio=0.5):
        # mask_ratio=0
        x = x.permute(0,2,1,3,4)
        _, _, T, _, _ = x.shape
        mask = TubeMaskingGenerator([T//2,self.img_size//self.patch_size,self.img_size//self.patch_size],mask_ratio)()
        mask = torch.tensor(mask).to(x.device).to(torch.bool)
        print("In",x.shape,mask_ratio)
       ...
       # print("emb",x.shape)
        ...
        cls_token = self.cls_token + pos[:, :1, :]
...
class PretrainVisionTransformer(nn.Module):
    ...
    def forward(self, x, mask_ratio=0.5):
        _, _, T, _, _ = x.shape
        cls,x = self.encoder(x, mask_ratio) # [B, N_vis, C_e]
        return cls
...
if __name__ == "__main__":
    x = torch.rand([16,3,8,224,224]).cuda()
    model = pretrain_videomae_base_patch16_224().cuda()
    tmp = model(x)
    print(tmp.shape)
